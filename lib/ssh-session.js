...
SSHSession.prototype.connect = function(connectTimeout) {
    return new Promise((resolve, reject) => {
        ...
        if (this.stun || this.turn) {
            ...
            firstCall = this.unifi.getSshTurnServers().then((data)=>{
                return this.unifi.buildSSHSession(this.mac, this.uuid, "-1", data.stun, data.turn, data.username, data.password, this.site);
            });
        }
        firstCall.then(() => {
