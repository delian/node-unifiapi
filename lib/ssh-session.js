    ...
    return new Promise((resolve, reject) => {
        ...
        if (this.stun || this.turn) {
            ...
            firstCall = this.unifi.getTurnCredentials().then((data) => {
                ...
                let stun = d.uris.filter(n => n.match(/stun:/)).shift();
                let turn = d.uris.filter(n => n.match(/turn:/)).shift();
                stun = stun ? stun.replace(/\?.*/,'') : stun;
                turn = turn ? turn.replace(/\?.*/,'') : turn;
                this.stun = stun;
                this.turn = turn;
                this.username = d.username;
                this.password = d.password;
                return this.unifi.buildSSHSession(this.mac, this.uuid, "-1", stun, turn, d.username, d.password, this.site);
            });
        }
        firstCall.then(() => {
