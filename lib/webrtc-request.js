...
WebRTCRequest.prototype.RTCPeerConnection = function(iceServers) {
    ...
    this.peer.ondatachannel = (event, a, b, c) => {
        debug('ondatachannel', event, a, b, c);
        this._datachannel.forEach((n) => n(event)); // Callback all waiters
    };
};
...
WebRTCRequest.prototype.openChannel = function(name) {
    return new Promise((resolve, reject) => {
        debug('WEBRTC_OPEN_CHANNEL', name);
        let channel = this.peer.createDataChannel(name);
        channel.onopen = (event) => {
            debug('channel, on open', event, channel.readyState);
            if (channel.readyState === "open") resolve();
        };
        channel.onmessage = (msg) => {
            debug('channel, on message', msg);
        };
        channel.onerror = (event) => {
            debug('channel, on error', event);
            reject();
        };
        channel.onclose = (event) => {
            debug('channel, on close', event);
